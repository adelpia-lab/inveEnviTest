...
export default function LowTempSettingPanel({ 
  ...
  const [form, setForm] = useState<LowTempSetting>(defaultSettings);
  ...
  useEffect(() => {
    console.log("🚀 LowTempSettingPanel component mounting");
  ...
  useEffect(() => {
    ...
    const handleMessage = (event) => {
      ...
      if (typeof message === 'string' && message.startsWith('Low temp settings read:')) {
        console.log("📥 Processing low temp settings read response from server");
        try {
          const match = message.match(/Low temp settings read: (.*)/);
          if (match && match[1]) {
            const readSettings = JSON.parse(match[1]);
            console.log('📥 Received low temp settings from server:', readSettings);
            
            const result = lowTempSchema.safeParse(readSettings);
            if (result.success) {
              setForm(readSettings);
              setIsReading(false);
              console.log('✅ Low temp settings loaded successfully from server');
            } else {
              console.log('❌ Server returned invalid low temp settings');
              setIsReading(false);
            }
          }
        } catch (error) {
  ...
  const handleRead = () => {
    ...
    try {
      const message = `[READ_LOW_TEMP_SETTINGS]`;
      wsConnection.send(message);
      console.log('📤 Sent READ_LOW_TEMP_SETTINGS command to server');
    } catch (error) {
  ...
  const handleSave = () => {
    ...
    if (wsConnection && wsConnection.readyState === WebSocket.OPEN) {
      try {
        ...
        wsConnection.send(message);
        console.log('📤 Sent low temp settings to server:', form);
      } catch (error) {
  ...
  return (
    ...
    <div className="flex gap-2">
      <Button
        ...
        onClick={handleRead}
        size="large"
        disabled={isReading || !isLowTempEnabled}
        sx={{ 
          width: '60px',
          opacity: (isReading || !isLowTempEnabled) ? 0.3 : 1,
          cursor: (isReading || !isLowTempEnabled) ? 'not-allowed' : 'pointer',
          backgroundColor: (isReading || !isLowTempEnabled) ? '#e0e0e0' : 'transparent',
          color: (isReading || !isLowTempEnabled) ? '#666' : 'inherit',
          pointerEvents: (isReading || !isLowTempEnabled) ? 'none' : 'auto'
        }}
      >
        READ
      </Button>
      <Button
        ...
        sx={{ 
          width: '60px',
          opacity: (isLoading || !isLowTempEnabled) ? 0.3 : 1,
          cursor: (isLoading || !isLowTempEnabled) ? 'not-allowed' : 'pointer',
          backgroundColor: (isLoading || !isLowTempEnabled) ? '#e0e0e0' : 'transparent',
          color: (isLoading || !isLowTempEnabled) ? '#666' : 'inherit',
          pointerEvents: (isLoading || !isLowTempEnabled) ? 'none' : 'auto'
        }}
